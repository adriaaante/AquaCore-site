...
## Рабочие правила сессии (применять всегда)

Эти правила грузятся автоматически в начале каждой сессии — это и есть «автомат».

1. **Копи знания о репозитории.** Если по ходу работы всплыло что-то неочевидное
   и полезное для будущего (скрытые цепочки, грабли, «почему так», где конфиг,
   нестандартные команды) — зафиксируй это в `CLAUDE.md`. Подробности и формат —
   в скилле **`repo-notes`** (`.claude/skills/repo-notes`). Кратко, только неочевидное;
   секреты не записывать. Память переживает сессии только в git → коммить.

2. **Код ради задачи, а не ради кода.** Каждая изменённая строка должна закрывать
   требование. Никакого drive-by рефакторинга, обёрток «на всякий случай» и
   «красоты» без влияния на корректность. Не можешь связать строку с задачей — удали.

3. **Рекомендуй, а не просто спрашивай.** Когда нужно моё решение — сначала сам
   разбери варианты и предложи рекомендацию («Развилка А/Б, плюсы-минусы, рекомендую А
   потому что…»). Очевидное решай сам. Несколько вопросов группируй в один.

4. **Доказывай, что баг реальный.** Не «улучшай» работающий код. Чини только баги,
   которые можешь воспроизвести (вход → ожидание → факт). Не уверен — не трогай.

5. **Анализ влияния перед мержем/деплоем.** Кто зависит от изменённых файлов?
   Не сломаны ли контракты (API/типы/формат ответа send.php)? Прогони проверки.
   Для этого сайта «проверка» — это блок «Проверка после изменений» ниже.

6. **Не рационализируй незавершённое.** Не закрывай задачу фразами «pre-existing»,
   «out of scope», «потом». Если что-то не доделано или проверка не пройдена —
   так и скажи, не выдавай за «готово».
