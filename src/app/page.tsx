    ...
    <section className="section bg-slate-50">
      <div className="container-x grid min-w-0 items-center gap-12 lg:grid-cols-2 [&>*]:min-w-0">
        <Reveal>
          ...
          <ul className="mt-6 space-y-3">
            {[
              "Вход через Telegram в одно касание — без приложений и паролей",
              "Ссылка постоянная и безопасная: без подтверждения по ней не войти",
              "QR-код на кассе превращает разовых посетителей в постоянных",
              "Модули «Клиентский кабинет» и «Уведомления (Telegram)» включаются одной галочкой",
            ].map((t) => (
              <li key={t} className="flex items-start gap-3 text-ink-soft">
                <span className="mt-0.5 grid h-5 w-5 place-items-center rounded-full bg-brand-50 text-brand-600">
                  <Icons.check className="h-3.5 w-3.5" />
...
function Roles() {
  const roles = [
    { icon: "chart" as const, t: "Управляющий", d: "Аналитика, цены, сотрудники, лояльность и настройки. Вход и восстановление доступа — по email с подтверждением почты." },
...
function Quality() {
  const items = [
    ...
    { icon: "bell" as const, t: "Чек в Telegram с бонусами", d: "После оплаты клиент получает в Telegram: сумму, начисленные бонусы и срок их действия. Лояльность работает сама." },
  ];
  return (
